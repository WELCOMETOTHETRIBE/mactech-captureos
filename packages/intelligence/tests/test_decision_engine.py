...
def test_fixture_1_cmmc_readiness_prime_now():
    r = decide(
        DecisionInputs(
            ...
            has_direct_cyber=True,
            relevance_weight=40,
            estimated_value_high=150_000,
...
def test_fixture_4_design_build_frcs_sub_identified():
    r = decide(
        DecisionInputs(
            ...
            notice_type="Solicitation",
            has_frcs_ot=True,
            has_construction_context=True,
            naics_is_construction=True,
            estimated_value_high=50_000_000,
            prime_targets_count=3,
            completeness="all_accessible",
            has_page_evidence=True,
            hard_barriers=frozenset({"BONDING_GAP"}),
        )
    )
...
def test_fixture_5_hvac_bas_not_discarded():
    r = decide(
        DecisionInputs(
            ...
            as_of=TODAY,
            notice_type="Solicitation",
            has_facility_adjacency=True,
            has_construction_context=True,
            naics_is_construction=True,
            estimated_value_high=8_000_000,
            completeness="description_only",
        )
    )
...
def test_fixture_6_no_scope_no_bid():
    r = decide(
        DecisionInputs(
            ...
            naics_is_construction=True,
            has_construction_context=True,
            estimated_value_high=5_000_000,
        )
    )
    assert r.pursuit_lane == "NO_BID"
...
def test_fixture_8_vehicle_blocked_sub_path():
    r = decide(
        DecisionInputs(
            ...
            notice_type="Solicitation",
            has_direct_cyber=True,
            estimated_value_high=1_000_000,
            completeness="all_accessible",
...
def test_fixture_12_division_25_high_sub_relevance():
    r = decide(
        DecisionInputs(
            ...
            naics_is_construction=True,
            has_page_evidence=True,
            estimated_value_high=12_000_000,
            prime_targets_count=0,
            completeness="all_accessible",
        )
    )
    assert r.pursuit_lane == "SUB_TO_PRIME_NOT_YET_IDENTIFIED"
